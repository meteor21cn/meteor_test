...
*/
    ...
    {
        //add by fzy
         if(response->content_type == CONTENT_TYPE_VIDEO) {
            char real_uri[1024];

                 //real_uri_start   real_uri_end
            int len = request->real_uri_end - request->real_uri_start + 1;
            strncpy(real_uri, request->real_uri_end, len);
            meteor_str_t uri;
            uri.data = real_uri;
            uri.length = len;
            save_to_cache_file(process, uri, con);
        }
