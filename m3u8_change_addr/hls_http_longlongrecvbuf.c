...
int main()
{
    ...
    recv_buf_t r;
    r.parsed1 = r.start = input;
    r.recved1 = r.end = input + strlen(input);

    chgAddr_buf_t h;
    h.pos = h.start = m3u_file;
    h.last = m3u_file;
    h.end = m3u_file + sizeof(m3u_file) - 1;

for(;r.parsed1<r.end;)
{
    chgAddr_long_file(&r,&h,NULL);
