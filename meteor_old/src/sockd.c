...
int convert_domain_to_ipaddr(socks_worker_process_t *process, socks_addr_t *addr)
{
	...
	if(node != NULL)
	{
		...
		sys_log(LL_DEBUG, "[ %s:%d ] ip of %s has existed in cache", __FILE__, __LINE__,domain_remote->domain);
		return 1;
	}
	...
	if(ret == 0)
	{
		...
		sys_log(LL_DEBUG, "[ %s:%d ] domain of %s will connect to %s", __FILE__, __LINE__, domain->domain, inet_ntoa(addr->ipv4));
...
void init_worker_process( socks_worker_process_t *process, socks_worker_config_t *w_config)
{
	...
	rb_list_init( &process->rb_node_pool, process->config->max_sessions*2 + 
		process->config->udp_port_end - process->config->udp_port_start + 1 + MAX_DOMAIN); //w_config->max_sessions
	// 申请订单的缓存空间
	...
	//init domain pool
	domain_pool_init(process, MAX_DOMAIN);
