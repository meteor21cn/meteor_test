	...
	while( node )
	{
		...
		if( session->close_stamp< expired ){
			...
			if( session->udp_remote){
				...
				session_node = rb_tree_search(&process->udp_addr_session_cache, &key);
				if (!session_node){
					sys_log(LL_ERROR, "[ %s:%d ] can not find %ld in udp_addr_session_cache", __FILE__, __LINE__, 
						session);
				}
				else{
					rb_erase(session_node, &process->udp_addr_session_cache);
				}
...
void init_worker_process( socks_worker_process_t *process, socks_worker_config_t *w_config)
{
	...
	// 申请红黑树节点的缓存空间
	rb_list_init( &process->rb_node_pool, process->config->max_sessions*2 + process->config->udp_port_end - process->config->udp_port_start + 1 ); //w_config->max_sessions
	// 申请订单的缓存空间
	...
	rb_tree_init_for_long_key( &process->udp_addr_session_cache);
...
void worker_process_exit( socks_worker_process_t *process )
{
	...
	rb_tree_destory( &process->udp_addr_session_cache, NULL);
