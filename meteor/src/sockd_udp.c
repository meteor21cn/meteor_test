	...
	socks_udp_connection_t * udp_remote_conn, int up_direct )
{
	...
	//insert udp_addr_session_cache, key: session
	rb_node_t * node = rb_list_pop( &process->rb_node_pool );
	if( !node ){
		sys_log(LL_ERROR, "[ %s:%d ] no memory for udp_addr_session_cache rb_node", __FILE__, __LINE__);
		return -1;
	}
	
	memcpy(&node->key.lkey, &udp_remote_conn->session, sizeof(udp_remote_conn->session));
	ret = rb_tree_insert_node( &process->udp_addr_session_cache, node, 0 );
	if (ret < 0)
	{
		sys_log(LL_ERROR, "[ %s:%d ] insert into udp_addr_session_cache failed !", __FILE__, __LINE__);
		return -1;
	}
...
void _udp_data_transform_cb_2( socks_worker_process_t *process, int fd, int events,  void *arg)
{
	...
	if( events & EPOLLIN)
	{
		...
		if(up_direct){
			...
			rb_node_t * node = rb_tree_search(&process->udp_addr_session_cache, &key);
			if (!node){
