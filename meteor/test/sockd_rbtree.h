...
#ifndef	_SOCKD_RBTREE_H
#define	_SOCKD_RBTREE_H
...
// added by jimmy zhou, add rb_key_u and modify rb_node, rb_root
union rb_key_u
{
	long lkey;
	void *pkey;
...
typedef union rb_key_u rb_key_t;
...
#define	RB_BLACK	1
	...
	// the following 2 lines added by jimmy zhou
	rb_key_t key;
	void *data;
...
struct rb_root
{
	...
	// the following 2 lines added by jimmy zhou
	long (*key_cmp)( rb_key_t *p, rb_key_t *q);
	size_t size;
};
...
// the following 2 line is added by jimmy zhou
#define rb_is_pool(r)   ((r)->rb_parent_color & 2)
#define rb_set_pool(r)  do { (r)->rb_parent_color |= 2; } while (0)
...
{
	// the following 2 line is modified by jimmy zhou, hold no.2 bit no change
	rb->rb_parent_color &= 2;
...
extern struct rb_node * _rb_node_calloc();
...
{
	//modified by jimmy zhou, change assign value to macro operate.
...
// the following is added by jimmy zhou
...
int rb_list_add( struct rb_list *, struct rb_node *);
